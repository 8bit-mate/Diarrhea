...
public class Packer(int numFilesReserved, string regExFilter)
{
    ...
    private static bool CheckIfValidIntValue(long value)
    {
    ...
    private FileEntry[] CreateFilesTable(FileInfo[] files)
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            if (!CheckIfValidIntValue(files[i].Length))
            {
                throw new FileSizeOverflowException(name);
            }

            var size = (int)files[i].Length;
            arr[i] = new FileEntry(name, offset, size);

            if (!CheckIfValidIntValue(offset += size))
            {
                throw new FileOffsetOverflowException(name);
            }
